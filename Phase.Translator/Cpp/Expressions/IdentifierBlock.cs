    ...
    public class IdentifierBlock : AutoCastBlockBase<IdentifierNameSyntax>
    {
        protected override AutoCastMode DoEmitWithoutCast(CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            else
            {
                ...
                {
                    Write(Emitter.GetTypeName((ITypeSymbol)resolve.Symbol, false, false, CppEmitter.TypeNamePointerKind.NoPointer));
                    EmitterContext.ImportType((ITypeSymbol)resolve.Symbol);
                }
                else
                {
                    ...
                    {
                        Write(Emitter.GetTypeName(resolve.Symbol.ContainingType, false, false, CppEmitter.TypeNamePointerKind.NoPointer));
                        EmitterContext.ImportType(resolve.Symbol.ContainingType);
                        Write("::");
                    }
