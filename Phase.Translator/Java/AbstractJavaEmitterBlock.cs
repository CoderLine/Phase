...
{
    public abstract class AbstractJavaEmitterBlock : AbstractEmitterBlock
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();
        public JavaEmitterContext EmitterContext { get; set; }

        protected override IWriter Writer => EmitterContext.Writer;
        public JavaEmitter Emitter => EmitterContext.Emitter;
        ...
        protected AbstractJavaEmitterBlock(JavaEmitterContext context)
        {
            EmitterContext = context;
        }


        protected AbstractJavaEmitterBlock EmitTree(SyntaxNode value, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (convertedType != null && type != null && !type.Equals(convertedType))
            {
                ...
                switch (convertedType.SpecialType)
                {
                    ...
                    case SpecialType.System_UInt64:
                        ...
                        if (Emitter.IsIConvertible(type) && type.SpecialType == SpecialType.None)
                        {
                            WriteDot();
        ...
        protected void WriteEventType(INamedTypeSymbol delegateType, bool includeTypeParameters = true)
        {
            ...
            if (delegateMethod.Parameters.Length > 0 || !delegateMethod.ReturnsVoid)
            {
                ...
                if (includeTypeParameters)
                {
                    ...
                    {
                        if (i > 0) WriteComma();
                        var typeName = Emitter.GetTypeName(delegateMethod.Parameters[i].Type, false, false, true);
                        Write(typeName);
                    }
                    ...
                    if (!delegateMethod.ReturnsVoid)
                    {
                        ...
                        var typeName = Emitter.GetTypeName(delegateMethod.ReturnType, false, false, true);
                        Write(typeName);
                    }
    ...
    public abstract class AbstractJavaEmitterBlock<T> : AbstractJavaEmitterBlock where T : SyntaxNode
    {
        public T Node { get; set; }

        protected AbstractJavaEmitterBlock() : base(null)
        {
        }

        public virtual void Emit(JavaEmitterContext context, T node, CancellationToken cancellationToken = default(CancellationToken))
        {
            EmitterContext = context;
            Node = node;
